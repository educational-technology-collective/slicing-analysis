...
def extract_quiz_csv_from_sql(course, session, outdir):
    ...
    # quiz meta
    query = """SELECT 'id', 'parent_id', 'open_time', 'soft_close_time', 'hard_close_time', 'maximum_submissions', 'title', 'duration', 'quiz_type', 'proctoring_requirement', 'authentication_required', 'deleted', 'last_updated' UNION ALL SELECT * FROM (SELECT * FROM quiz_metadata WHERE parent_id = -1 AND open_time IS NOT NULL AND deleted = 0) AS temp2 """
